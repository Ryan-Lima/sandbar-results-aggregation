...
def eddy_metrics_calculator(max_surf, min_surf, gt, shps,site,raster_out=False,reatt=False,sep=False):
    ...
    fz_plane = np.ma.ones(np.ma.shape(fz_data))
    fz_plane = np.ma.masked_where(np.ma.getmask(fz_data),fz_plane)
    fz_plane = fz_plane*low
    fz_dod = fz_data -fz_plane
    fz_max_vol = np.ma.sum(np.ma.sum(fz_dod, axis=1),axis=0)*0.25**2
    
    #he_data
    he_data = np.ma.masked_where(eddy_data<=high,eddy_data)
    he_plane = np.ma.ones(np.ma.shape(he_data))
    he_plane = np.ma.masked_where(np.ma.getmask(he_data),he_plane)
    he_plane = he_plane*high
    he_dod = he_data -he_plane
...
def agg_hist_plot(max_index,min_index,survey_dates, site,shps,gt,out_root,reatt=False,sep=False):
    
    out_root = out_root + os.sep + 'Plots'
    ...
    affine = Affine.from_gdal(*gt)
    eddy = shps[0]
    arrays = zonal_stats(eddy,max_index,affine=affine,nodata=-9999,stats=['count'], raster_out=True)
    max_data = arrays[0]['mini_raster_array']
    mini_affine = arrays[0]['mini_raster_affine']
    
    del arrays
    arrays = zonal_stats(eddy,min_index,affine=affine,nodata=-9999,stats=['count'], raster_out=True)
    min_data = arrays[0]['mini_raster_array']
    del arrays
    
    gt = Affine.to_gdal(mini_affine)
    xres = gt[1]
    yres = gt[5]
    ...
    # to go to center coordinates
    xmin = gt[0] + xres * 0.5
    xmax = gt[0] + (xres * max_data.shape[1]) - xres * 0.5
    ymin = gt[3] + (yres * max_data.shape[0]) + yres * 0.5
    ymax = gt[3] - yres * 0.5
    ...
    m = Basemap(projection='merc', 
                    epsg=26949, 
                    llcrnrlon=np.min(glon)-0.0003, 
                    llcrnrlat=np.min(glat)-0.0003,
                    urcrnrlon=np.max(glon)+0.0003, 
                    urcrnrlat=np.max(glat)+0.0003,ax=ax)
    ...
    try:
        max_data = max_data.filled(np.nan)
    except:
        pass
    m.contourf(x,y,max_data.T, cmap=cmap,norm=norm)#
    ...
    fig.savefig(out_root + os.sep + site + '_Agg_hist_max.png')
    ...
    m = Basemap(projection='merc', 
                ...
                llcrnrlat=np.min(glat)-0.0003,
                urcrnrlon=np.max(glon)+0.0003, 
                urcrnrlat=np.max(glat)+0.0003,ax=ax1)
    m.wmsimage(server='http://grandcanyon.usgs.gov/arcgis/services/Imagery/ColoradoRiverImageryExplorer/MapServer/WmsServer?', layers=['3'], xpixels=1000)
    x,y = m.projtran(glon, glat)
    try:
        min_data = min_data.filled(np.nan)
    except:
        pass
    m.contourf(x,y,min_data.T, cmap=cmap,norm=norm)#
    ...
    fig1.tight_layout()
    fig1.subplots_adjust(top = 0.968) 
    fig1.savefig(out_root + os.sep + site + '_Degrade_hist_min.png')
...
if __name__ =='__main__':
    ...
    for i in single_eddy_sites:
        ...
        agg_hist_plot(max_index,min_index,survey_dates, site,shps,gt,out_root,reatt=False,sep=False)
    ...
    for i in two_bar_sites:
        ...
        survey_dates = get_date_list(files)
        agg_hist_plot(max_index,min_index,survey_dates, site,shps,gt,out_root,reatt=True,sep=False)
        if 'result' in locals():
    ...
    for i in two_bar_sites:
        ...
        survey_dates = get_date_list(files)
        agg_hist_plot(max_index,min_index,survey_dates, site,shps,gt,out_root,reatt=False,sep=True)
        if 'result' in locals():
    ...
    df.to_csv(out_root + os.sep +'Eddy_Max_Vol_Wedge_Method.csv', sep = ',')
